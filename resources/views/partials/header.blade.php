<header class="sticky top-0 z-50" x-data="{ mobileMenu: false }">
    ...
    {{-- ============================================================
         DESKTOP: fila única — Logo | Categorías+Búsqueda | Cuenta | Carrito
         ============================================================ --}}
    <div class="hidden md:block py-3 bg-white border-b border-border shadow-sm">
        <div class="container mx-auto px-4">
            <div class="flex items-center gap-6">
                ...
                {{-- Categorías + Búsqueda --}}
                <div class="flex flex-1 max-w-2xl h-10">
                    {{-- Dropdown Categorías --}}
                    <div class="flex items-stretch flex-shrink-0 relative" x-data="{ open: false }">
                ...
                {{-- Cuenta + Carrito (siempre a la derecha) --}}
                ...
                @else
                    @if(auth()->user()->isAdmin() || auth()->user()->isVendedor())
                        <a href="{{ route('admin.home') }}"
                           class="flex items-center gap-2 text-primary hover:text-primary/60 transition-colors font-semibold text-sm flex-shrink-0">
    ...
    <div class="md:hidden">
        ...
        <div class="bg-white border-b border-border shadow-sm pb-3 pt-2">
            <div class="container mx-auto px-4">
                <div class="flex h-10">
                    {{-- Botón Categorías (abre mobile menu) --}}
                    <button @click="mobileMenu = !mobileMenu"
                            class="flex items-center gap-1.5 rounded-l-full border border-r-0 border-border bg-muted px-4 text-sm text-foreground font-medium whitespace-nowrap transition-colors hover:bg-muted/80 flex-shrink-0">
                        <svg class="w-4 h-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
                        </svg>
                        Categorías
