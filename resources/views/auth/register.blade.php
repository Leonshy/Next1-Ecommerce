    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="px-8 pb-8">
            <form method="POST" action="{{ route('register') }}" class="space-y-4"
                  @submit.prevent="if (!acceptTerms) { termsError = true; return; } termsError = false; $el.submit()">
                @csrf
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input id="name" name="name" type="text"
                               ...
                               class="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-colors @error('name') border-red-400 @enderror">
                    </div>
                    @error('name')
                ...
                <div class="space-y-3 pt-1">
                    <label class="flex items-start gap-3 cursor-pointer">
                        <input type="checkbox" x-model="acceptTerms" @change="if (acceptTerms) acceptPromotions = true"
                               class="mt-0.5 w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary/30 flex-shrink-0">
                        <span class="text-sm text-gray-600 leading-snug">
                            Acepto los
                            <a href="#" class="text-primary font-medium hover:underline" target="_blank">Términos y Condiciones</a>
                            <span class="text-red-500"> *</span>
                        </span>
                    </label>
                    <p x-show="termsError" class="text-xs text-red-500 -mt-1 pl-7" style="display:none">
                        Debes aceptar los términos y condiciones
                    </p>

                    <label class="flex items-start gap-3 cursor-pointer">
                        <input type="checkbox" name="accept_promotions" x-model="acceptPromotions"
                               class="mt-0.5 w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary/30 flex-shrink-0">
                        <span class="text-sm text-gray-600 leading-snug">
                            Quiero recibir promociones y ofertas exclusivas
                        </span>
                    </label>
                </div>
                ...
                {{-- Submit --}}
                <button type="submit"
                        class="w-full bg-primary text-white py-2.5 rounded-lg font-semibold text-sm hover:bg-primary-light transition-colors mt-2 disabled:opacity-50 disabled:cursor-not-allowed"
                        :class="!acceptTerms ? 'opacity-60' : ''">
                    Crear Cuenta
                </button>
            </form>
            ...
            <div class="mt-4">
                <a href="{{ route('auth.google') }}"
                   ...
                   class="flex items-center justify-center gap-3 w-full border border-gray-200 rounded-lg py-2.5 text-sm font-medium text-gray-700 transition-colors">
                    <template x-if="!captchaDone">
                ...
                <p x-show="!captchaDone" class="mt-2 text-xs text-gray-400 text-center" style="display:none">
                    Completá la verificación de seguridad para habilitar esta opción
                </p>
            </div>
            ...
            <p class="text-center text-sm text-gray-500 mt-5">
                ¿Ya tienes cuenta?
                <a href="{{ route('login') }}" class="text-primary font-semibold hover:underline">Inicia sesión</a>
            </p>
        </div>
    </div>
